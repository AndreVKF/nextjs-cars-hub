...
import { updateSearchParams } from '@/common/functions'
          ...
          >
            <Listbox.Options className={"custom-filter__options"}>
              {options.map((option, idx) => {
                return (
                  <Listbox.Option key={idx} value={option} className={({active}) => `relative cursor-default select-none py-2 px-4 ${active ? 'bg-primary-blue text-white' : 'text-gray-900'}`}> 
                    {({ selected }) => (
